...
public class Steering
{
    ...
    {
        Vector3 force = CalculateSteering(agent, (target.transform.position - target.transform.position));
        return force;
    }
    ...
    public static Vector3 Wander(AutonomousAgent agent)
    {
        ...
        Debug.DrawRay(agent.transform.position, Vector3.forward + point, Color.red);
    ...
    {
        // Get center posistion of nieghbors
    ...
    public static Vector3 Seperation(Agent agent, GameObject[] neighbors, float radius)
    {
        ...
        foreach (GameObject neighbor in neighbors)
        {
            ...
            Vector3 direction = neighbor.transform.position - agent.transform.position; //< get vector that points to neighbor from agent >;
